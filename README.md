...
- Optional modules: equal highs/lows tracking (EQH/EQL, CantoLab "EQ & RE" structure) with optional raid-arming, a higher-timeframe FVG overlay (5m / 15m / 1h / 4h / daily), session boxes, raid labels, the Macro Dealing Range premium/discount zones (the standalone indicator below, built in as a group 05 toggle, on by default), and Williams-fractal swing-point marks (group 06, default 10 periods).
